...
<%
    ...
    int    zSel     = request.getAttribute("zSeleccionado") != null
                      ? (int)request.getAttribute("zSeleccionado") : 0;
    ...
    // ── Neutrones actuales ───────────────────────────────────────────────────
    int nAct = request.getAttribute("neutronesActuales") != null
               ? (int)request.getAttribute("neutronesActuales") : 0;

    // ── Datos del isótopo actual ─────────────────────────────────────────────
    String nomIsoAct    = request.getAttribute("nombreIsotopoActual") != null
                          ? (String)request.getAttribute("nombreIsotopoActual")  : "—";
    String estabAct     = request.getAttribute("estabilidadActual")   != null
                          ? (String)request.getAttribute("estabilidadActual")    : "—";
    double abundAct     = request.getAttribute("abundanciaActual")    != null
                          ? (double)request.getAttribute("abundanciaActual")     : 0.0;
    double masaIsoAct   = request.getAttribute("masaIsotopicaActual") != null
                          ? (double)request.getAttribute("masaIsotopicaActual")  : 0.0;
    int    numMasicoAct = request.getAttribute("numeroMasicoActual")  != null
                          ? (int)request.getAttribute("numeroMasicoActual")      : 0;
    ...
    String       nomIsoObj         = request.getAttribute("nomIsotopoObjetivo") != null
                                     ? (String)request.getAttribute("nomIsotopoObjetivo") : "";
    int          neutronesObjetivo = request.getAttribute("neutronesObjetivo") != null
                                     ? (int)request.getAttribute("neutronesObjetivo") : 0;
    ...
    // ── Lista de elementos para tabla periódica ──────────────────────────────
    ...
    // Formato abundancia y masa
    String abundStr = abundAct > 0
        ? String.format("%.4f%%", abundAct)
        : (hayElemento ? "No registrada" : "—");
    String masaStr = masaIsoAct > 0
        ? String.format("%.5f", masaIsoAct)
        : (hayElemento ? String.format("%.5f", (double)ebSel.getMasaAtomica()) : "—");
%>
<!DOCTYPE html>
<html lang="es">
<head>
...
.sim{background:var(--panel);border:3px solid var(--border);border-radius:28px;
    box-shadow:0 8px 32px rgba(40,70,160,.12);width:100%;max-width:1100px;
...
/* ── GRID (2 cols) ── */
.body-grid{display:grid;grid-template-columns:1fr 390px;gap:14px;align-items:start}
.col-left{display:flex;flex-direction:column;gap:10px}
.col-right{display:flex;flex-direction:column;gap:10px}

/* Título isótopo */
.iso-titulo{font-family:var(--ft);font-size:26px;font-weight:900;color:#1a2848;
    text-align:center;letter-spacing:3px;
    background:#edf2ff;border:2.5px solid var(--border);border-radius:14px;
    padding:10px 16px;transition:all .3s}
.iso-titulo.activo{color:var(--purple);border-color:var(--purple);
    background:#f5eeff;box-shadow:0 0 0 3px rgba(155,93,229,.15)}

/* Carta elemento */
.carta-sel{background:#edf2ff;border:2.5px solid var(--border);border-radius:14px;
    padding:10px 12px;display:flex;align-items:center;gap:10px}
.cs-nums{display:flex;flex-direction:column;align-items:center;justify-content:space-between;
    padding-right:10px;border-right:2px solid var(--border);min-width:36px}
.cs-mas,.cs-z{font-size:20px;font-weight:900;color:#1a2848;line-height:1}
.cs-info{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px}
.cs-sim{font-family:var(--ft);font-size:42px;font-weight:900;color:var(--blue);line-height:1}
.cs-sim.vacio{color:#b8c8e8}
.cs-nom{font-size:11px;font-weight:700;color:#7a8cb0}

/* Info isótopo */
.iso-info{background:#edf2ff;border:2px solid var(--border);border-radius:14px;
    padding:12px 16px;display:flex;flex-direction:column;gap:8px}
.iso-row{display:flex;align-items:center;justify-content:space-between}
.iso-lbl{font-size:12px;font-weight:800;color:#7a8cb0;letter-spacing:.5px;text-transform:uppercase}
.iso-val{font-size:15px;font-weight:900;color:#1a2848;
    background:#fff;border:1.5px solid var(--border);border-radius:8px;
    padding:3px 14px;min-width:90px;text-align:center;transition:all .3s}
.iso-val.estable  {color:var(--green-d);background:#d2f5e2;border-color:var(--green)}
.iso-val.inestable{color:var(--red-d);  background:#fde0e0;border-color:var(--red)}
...
/* Controles neutrones */
.neutron-ctrl{display:flex;flex-direction:column;gap:8px}
.neutron-ctrl-title{font-size:12px;font-weight:800;color:#7a8cb0;
    letter-spacing:.8px;text-transform:uppercase;text-align:center}
.neutron-row{display:flex;align-items:center;justify-content:center;gap:14px}
.btn-n{width:56px;height:56px;border-radius:50%;border:none;
...
.neutron-lbl{background:var(--neutron);color:#4a3000;border-radius:50px;
    padding:8px 22px;font-size:13px;font-weight:800;white-space:nowrap}
.neutron-count{font-size:28px;font-weight:900;color:#1a2848;min-width:44px;text-align:center}

/* Átomo */
.atomo-w{display:flex;align-items:center;justify-content:center}
.atomo-svg{width:240px;height:240px;overflow:visible}

/* ══ COL DERECHA ══ */
/* Tabla periódica */
.tabla-titulo{font-size:12px;font-weight:800;color:#7a8cb0;
    letter-spacing:.8px;text-align:center;text-transform:uppercase;margin-bottom:4px}
.tabla-periodica{display:grid;grid-template-columns:repeat(18,1fr);gap:2px}
.elem-cell{aspect-ratio:1;border-radius:5px;border:1.5px solid var(--border);
    background:#edf2ff;cursor:pointer;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    transition:all .15s;position:relative}
.elem-cell:hover{background:#d0e0ff;border-color:var(--blue);transform:scale(1.14);z-index:2}
.elem-cell.seleccionado{background:var(--blue);border-color:var(--blue-d);
    box-shadow:0 0 0 2px rgba(74,134,245,.5)}
.elem-cell.seleccionado .ec-sim,.elem-cell.seleccionado .ec-z{color:#fff}
.elem-cell.vacia{background:transparent;border-color:transparent;cursor:default;pointer-events:none}
.ec-z  {font-size:6px;font-weight:800;color:#9aa8c8;line-height:1}
.ec-sim{font-size:9px;font-weight:900;color:#1a2848;line-height:1}
.elem-cell.s-block{background:#d2f5e2;border-color:#2a8a4e}
.elem-cell.s-block .ec-sim{color:#1a4a2e}
.elem-cell.noble   {background:#fde0e0;border-color:var(--red)}
.elem-cell.noble .ec-sim{color:var(--red-d)}

/* Carta objetivo */
.obj-box{background:#edf2ff;border:2.5px solid var(--teal);border-radius:18px;padding:14px 16px}
...
.obj-iso-nom{font-family:var(--ft);font-size:22px;font-weight:900;color:#1a2848;
    text-align:center;margin-bottom:6px}
.obj-rows{display:flex;flex-direction:column;gap:5px}
.obj-row{display:flex;justify-content:space-between;align-items:center;
    background:#fff;border-radius:8px;padding:4px 10px}
.obj-lbl{font-size:11px;font-weight:800;color:#7a8cb0;text-transform:uppercase}
.obj-val{font-size:13px;font-weight:900;color:#1a2848}
.obj-placeholder{text-align:center;padding:14px;color:#aab8d0;font-size:12px;font-weight:700}

/* Abundancia */
.abund-box{background:linear-gradient(135deg,#f0f5ff 0%,#e8f5ee 100%);
...
.abund-val{font-size:28px;font-weight:900;color:var(--green-d);font-family:var(--ft);transition:all .4s}
...
<body>
<%-- Formulario principal: envía accion + numeroAtomico (Z) en vez de idElemento --%>
...
<div class="sim">
  ...
  <div class="body-grid">
    ...
    <div class="col-left">
      ...
      <!-- Nombre isótopo actual -->
      <div class="iso-titulo <%= hayElemento?"activo":"" %>" id="isoTitulo">
        <%= hayElemento ? nomIsoAct : "← Selecciona un elemento" %>
      </div>

      <!-- Carta elemento seleccionado -->
      <div class="carta-sel">
        <div class="cs-nums">
          <span class="cs-mas" id="csMas"><%= hayElemento ? numMasicoAct : 0 %></span>
          <span class="cs-z"   id="csZ"><%= protones %></span>
        </div>
        <div class="cs-info">
          <span class="cs-sim <%= simSel.isEmpty()?"vacio":"" %>" id="csSim"><%= simSel.isEmpty()?"?":simSel %></span>
          <span class="cs-nom" id="csNom"><%= nomSel %></span>
        </div>
      </div>
      ...
      <div class="iso-info">
        <div class="iso-row">
          ...
          <span class="iso-val" id="isoA"><%= hayElemento ? numMasicoAct : "—" %></span>
        </div>
        <div class="iso-row">
      ...
      <!-- Controles SOLO neutrones -->
      <div class="neutron-ctrl">
        <div class="neutron-ctrl-title">⚛ Configurar Neutrones</div>
        <div class="neutron-row">
          <button class="btn-n plus"  id="btnNMas"
                  <%= !hayElemento?"disabled":"" %>
                  onclick="enviar('incrementarNeutrones')">+</button>
          <div class="neutron-lbl">Neutrones</div>
          <span class="neutron-count" id="nCount"><%= nAct %></span>
          <button class="btn-n minus" id="btnNMenos"
                  <%= !hayElemento?"disabled":"" %>
      ...
      <!-- Átomo visual -->
      <div class="atomo-w">
        <svg class="atomo-svg" viewBox="0 0 260 260">
          <circle cx="130" cy="130" r="118" fill="none" stroke="#c5d2ec" stroke-width="1.7" stroke-dasharray="7 4"/>
          <circle cx="130" cy="130" r="80"  fill="none" stroke="#c5d2ec" stroke-width="1.7" stroke-dasharray="7 4"/>
          <circle cx="130" cy="130" r="42"  fill="none" stroke="#beccde" stroke-width="1.4" stroke-dasharray="5 4"/>
          <g id="nucleoG"></g>
          <g id="electronG"></g>
        </svg>
      </div>
    ...
    <div class="col-right">
      ...
      <!-- Tabla periódica primeros 18 elementos -->
      <div>
        <div class="tabla-titulo">Tabla Periódica — Selecciona el elemento a trabajar</div>
        <div class="tabla-periodica">
          <%
            // Mapa posición (1-54) → ElementoBase
            // Grid 18 cols × 3 filas (períodos 1, 2, 3)
            java.util.Map<Integer,ElementoBase> posMap = new java.util.HashMap<>();
            if (elemPeriodica != null) {
                for (ElementoBase e : elemPeriodica) {
                    int z = e.getNumeroAtomico();
                    // Período 1: H→col1, He→col18
                    if (z==1)  posMap.put(1,  e);
                    if (z==2)  posMap.put(18, e);
                    // Período 2: Li-Be cols 1-2, B..Ne cols 13-18
                    if (z==3)  posMap.put(19, e);
                    if (z==4)  posMap.put(20, e);
                    if (z==5)  posMap.put(31, e);
                    if (z==6)  posMap.put(32, e);
                    if (z==7)  posMap.put(33, e);
                    if (z==8)  posMap.put(34, e);
                    if (z==9)  posMap.put(35, e);
                    if (z==10) posMap.put(36, e);
                    // Período 3: Na-Mg cols 1-2, Al..Ar cols 13-18
                    if (z==11) posMap.put(37, e);
                    if (z==12) posMap.put(38, e);
                    if (z==13) posMap.put(49, e);
                    if (z==14) posMap.put(50, e);
                    if (z==15) posMap.put(51, e);
                    if (z==16) posMap.put(52, e);
                    if (z==17) posMap.put(53, e);
                    if (z==18) posMap.put(54, e);
                }
            }
            // Clases de bloque
            java.util.Set<Integer> sSet = new java.util.HashSet<>(java.util.Arrays.asList(1,3,4,11,12));
            java.util.Set<Integer> nSet = new java.util.HashSet<>(java.util.Arrays.asList(2,10,18));

            for (int pos = 1; pos <= 54; pos++) {
                ElementoBase e = posMap.get(pos);
                if (e == null) {
          %>
              <div class="elem-cell vacia"></div>
          <%
                } else {
                    int zE = e.getNumeroAtomico();
                    String blk = nSet.contains(zE) ? "noble" : (sSet.contains(zE) ? "s-block" : "p-block");
                    String selCls = (zE == zSel) ? " seleccionado" : "";
          %>
              <div class="elem-cell <%= blk %><%= selCls %>"
                   title="<%= e.getNombre() %> (Z=<%= zE %>)"
                   onclick="seleccionarElemento(<%= zE %>)">
                <span class="ec-z"><%= zE %></span>
                <span class="ec-sim"><%= e.getSimbolo() %></span>
              </div>
          <%
                }
            }
      ...
      <!-- Carta isótopo objetivo (solo eval) -->
      <div class="obj-box <%= hayObjetivo?"":"vacio" %>">
        ...
        <div class="obj-placeholder">
          <%= modoEval ? "Cargando reto…" : "Inicia la evaluación para ver el isótopo objetivo" %>
        </div>
        <% } %>
      </div>
...
// Acciones: sin parámetro de partícula (solo neutrones)
function enviar(accion){
    document.getElementById('hdnA').value = accion;
    document.getElementById('frm').submit();
}
// Seleccionar elemento por Z (número atómico)
function seleccionarElemento(z){
...
function openModal(){
    ...
    const saved = sessionStorage.getItem('seaea4_desc_'+ST.retoId);
...
const GUIA = [
    {t:'¡Bienvenido a Configura tu Isótopo!',
     m:'Hola, soy AmazonAtom 🦁\nEn este escenario explorarás los ISÓTOPOS de los elementos.\n¡Los isótopos son átomos del mismo elemento con diferente número de neutrones!', btn:'Siguiente →'},
    {t:'¿Qué son los isótopos?',
     m:'⚛️ Isótopos: átomos con el mismo Z (protones) pero distinto A (número másico).\n\nEjemplo: Helio-3 (2p, 1n) y Helio-4 (2p, 2n).\nNombre: Elemento-A  →  Helio-5', btn:'Siguiente →'},
    {t:'Cómo usar este escenario',
     m:'1️⃣ Selecciona un elemento de la tabla periódica.\n2️⃣ Los protones y electrones se fijan automáticamente.\n3️⃣ Solo puedes modificar los NEUTRONES.\n4️⃣ El nombre del isótopo, la masa y la abundancia se actualizan en tiempo real.', btn:'Siguiente →'},
    {t:'Estabilidad y abundancia',
     m:'🟢 ESTABLE: el núcleo no se desintegra.\n🔴 INESTABLE: el núcleo es radiactivo.\n\n📊 La abundancia indica qué porcentaje de ese elemento en la naturaleza corresponde a este isótopo.', btn:'Siguiente →'},
    {t:'¡Listo para evaluarte!',
     m:'🏆 Presiona INICIAR EVALUACIÓN.\nEl sistema te pedirá configurar un isótopo específico.\nSelecciona el elemento correcto y ajusta los neutrones.\nNecesitas ≥ 80% para superar el escenario.', btn:'¡Entendido!'}
];
...
/* ── ÁTOMO VISUAL ── */
const R=7, NS='http://www.w3.org/2000/svg';
...
function dibujarNucleo(p,n){
    ...
    hexLayout(total).forEach((pos,i)=>{
        ...
        c.setAttribute('cx',130+pos.x); c.setAttribute('cy',130+pos.y); c.setAttribute('r',R);
...
function dibujarElectrones(e){
    ...
    const orbs=[{r:42,max:2},{r:80,max:8},{r:118,max:18}]; let rest=e;
    orbs.forEach(o=>{
        if(rest<=0) return; const en=Math.min(rest,o.max); rest-=en; const step=(2*Math.PI)/en;
        for(let i=0;i<en;i++){
            const a=step*i-Math.PI/2;
            const c=document.createElementNS(NS,'circle');
            c.setAttribute('cx',130+o.r*Math.cos(a)); c.setAttribute('cy',130+o.r*Math.sin(a));
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    dibujarElectrones(ST.p); // electrones = protones (átomo neutro)
    renderDots('dotsP', ST.p, 'd-p');
    renderDots('dotsN', ST.n, 'd-n');
    renderDots('dotsE', ST.p, 'd-e');

    if(ST.modoEval && ST.retoId){
        const storedId    = sessionStorage.getItem('seaea4_retoId');
        const storedTimer = parseInt(sessionStorage.getItem('seaea4_timer')||'0');
    ...
    <% if (tieneResult) { %>
    {
        ...
        setTimeout(()=>{
            mostrarRetro(titulo, msg, estado,
                nuDesc || null,
